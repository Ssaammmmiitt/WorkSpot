...
const Salary = () => {
  ...
  const memoizedFilteredSalaries = useMemo(
    () => filteredSalaries,
    [filteredSalaries]
  );
    ...
    <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4">
      ...
      <div className="mt-5">
        <div className="search-box p-2 text-center mb-2">
          ...
          <button
            onClick={handleReset}
            className="bg-Secondary text-white font-semibold px-8 py-2 rounded-sm mb-4 ml-4"
          >
            Reset
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-12 my-12 items-center animate__animated animate__slideInLeft">
          {memoizedFilteredSalaries.map(
            (data) =>
              data.salary !== " Salary 0.00 per month" && (
                <div
                  key={data.id}
                  className="shadow-lg px-6 py-8 rounded-lg transition-transform transform hover:scale-105"
                >
                  <h4 className='font-semibold text-xl'>{data.title}</h4>
                  <p className="my-2 font-medium text-[#00006f] text-lg">
                    {" "}
                    Salary : {data.salary}
                  </p>
                  <div className="flex flex-row gap-4">
                    ...
                    <svg
                      ...
                      className="icon icon-tabler icons-tabler-outline icon-tabler-currency-rupee-nepalese"
                    >
